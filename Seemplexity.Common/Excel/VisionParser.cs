...
// Type: Seemplexity.Common.Excel.VisionParser
...
{
  public class VisionParser : ITransferParser
  {
    ...
    private Hashtable FillExcelSheets(string fileName)
    {
      ...
      using (ExcelQueryFactory excelQueryFactory = new ExcelQueryFactory(fileName))
      {
        ...
        {
          RowNoHeader rowNoHeader = excelQueryFactory.WorksheetNoHeader(worksheetName).FirstOrDefault<RowNoHeader>((Expression<Func<RowNoHeader, bool>>) (r => (string) r[0] == "№" || (string) r[0] == "NN"));
          if (rowNoHeader != null)
          {
            ...
            {
              string upper = rowNoHeader[index].ToString().Trim().ToUpper();
              if (upper == "№" || upper == "NN")
                dictionary.Add("Id", index);
              else if (upper.Contains("ТУРИСТ"))
                dictionary.Add("FullName", index);
              else if (upper.Contains("КУРОРТ") || upper.Contains("КУРОРРТ"))
                dictionary.Add("Resort", index);
              else if (upper.Contains("ОТЕЛЬ"))
    ...
    private List<List<TouristTransferRow>> GetTourists(string fileName)
    {
      ...
      foreach (object key in (IEnumerable) hashtable.Keys)
      {
        ...
        {
          if (!((string) rowNoHeader[0] == string.Empty))
          {
            if (flag)
    ...
    public ExcelTransferModel Parse(string fileName)
    {
      ...
      excelTransferModel.Type = PartnerType.Vision;
